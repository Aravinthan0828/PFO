...
function pagePilling() {
    ...
    $('#pagepiling').pagepiling({
        ...
        afterLoad: function (anchorLink, index) {
            ...
            var zIndex = parseInt($activeSection.css('z-index'), 10);

            console.log('Current section z-index:', zIndex);

            if (zIndex === 7 || zIndex === 1 || zIndex === 6) {
                $('#logoMain').removeClass('text-dark').addClass('text-light');
                $('#tollNavBar').removeClass('toggler-active').addClass('toggler');
                $('#pp-nav li span').css('background-color', 'white');
                $('#pp-nav li .active span').css('background-color', '#e24727');
            } else {
                $('#pp-nav li span').css('background-color', 'black');
                $('#pp-nav li .active span').css('background-color', '#e24727');
                $('#logoMain').removeClass('text-light').addClass('text-dark');
                $('#tollNavBar').removeClass('toggler').addClass('toggler-active');
            }

            $('#custom-nav a').removeClass('active');
            $('#custom-nav a[data-index="' + index + '"]').addClass('active');
        },

        afterRender: function () {
            // Optional: Add logic if you want something to happen after initial render
        },
    });
}
